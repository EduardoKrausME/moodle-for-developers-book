...
No capítulo anterior trabalhamos com PHPUnit e vimos uma verdade importante: a maior parte da regra de negócio do plugin deve ser testada sem abrir navegador. Isso deixa a suíte rápida, previsível e boa para encontrar regressões em classes, banco, capabilities, Events, Tasks, Web Services e praticamente qualquer outra API do Moodle. Só que existe uma categoria de problema que PHPUnit não enxerga bem, porque o erro não está em uma função isolada, está na jornada inteira que o usuário executa pela interface.
...
## Unitário, integração e aceitação
...
## O ambiente Behat é outro Moodle
...
## O erro de `$CFG->behat_* precisa ser definido`
...
Depois da inicialização, uma forma de rodar Behat é:
...
```
php admin/tool/behat/cli/run.php --tags="@mod_checkpoint"
```
...
## Rodando uma feature específica
...
## Não transforme Behat em tabela de casos unitários
...
## Não crie dados de setup clicando pela interface
...
## Generator próprio do plugin para Behat
...
## Testar breadcrumb não é testar navegação inteira
...
## Gherkin deve ser legível por alguém que não viu o DOM
...
Não presuma que qualquer versão de Selenium ou do navegador funcionará com sua suíte. O ambiente de CI precisa fixar combinações conhecidas e atualizar dependências de forma consciente.
...
## Um custom step deve representar domínio
...
## Step de setup versus step de interface
...
## Não coloque assertion escondida em todo step
...
## Por que `sleep()` deixa teste lento e ainda frágil
...
## IDs não devem aparecer no Gherkin sem necessidade
...
## Auto-rerun não é solução para flakiness
...
## Teste precisa ser isolado para rodar em paralelo
...
## Acessibilidade nos testes de interface

Behat pode ajudar a garantir que elementos, rótulos e fluxos importantes continuem presentes, mas não substitui revisão de acessibilidade com teclado, contraste e tecnologias assistivas. Se o plugin introduz uma interface nova, inclua cenários que cubram os fluxos essenciais e faça a revisão de acessibilidade como parte do desenvolvimento, não apenas no fim do projeto.

## Steps existentes antes de steps customizados
...
## Não escrever step que chama step demais
...
## Dados aleatórios deixam falha difícil de reproduzir
...
## Cenário pequeno não significa cenário artificial
...
## O que não vale a pena testar com Behat
...
## O que vale muito testar com Behat
...
## Behat e JavaScript customizado

Se o plugin possui um módulo AMD que inicializa modal, busca dados e atualiza DOM, Behat é uma das formas de testar a composição inteira dentro do Moodle.
...
## Exemplo completo com `mod_checkpoint`
...
## Separando cenários para manter diagnóstico bom
...
## Custom step para resposta existente
...
## Screenshots devem ajudar, não substituir assertions
...
## Revisando um Behat em code review
