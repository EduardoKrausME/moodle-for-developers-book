# BACKUP E RESTORE - `backup/moodle2`
...
Neste capítulo vamos evoluir o `mod_checkpoint` dos capítulos anteriores, agora assumindo que ele possui configuração, respostas de alunos, arquivos anexados, notas e uma estrutura filha de critérios. A atividade será usada para explicar o caminho completo, desde `define_structure()` até `after_execute()`, incluindo `backup_nested_element`, `annotate_ids()`, `annotate_files()`, mappings de restore, dados dependentes de usuário e codificação de links. No fim, o objetivo é simples de dizer e trabalhoso de garantir: fazer backup no site A, restaurar no site B e continuar com a mesma atividade do ponto de vista funcional, mesmo que todos os IDs internos tenham mudado.

## O que realmente existe dentro de um `.mbz`
...
## Backup de curso não é backup do site
...
## Estrutura de arquivos para uma activity module
...
## `backup_nested_element` não é uma tabela
...
## Estrutura sem dados de usuário
...
## Anotar IDs não muda o valor no backup
...
## Backup task com múltiplos steps
...
## Usuário que não existe no destino
...
## Restaurando respostas de usuários
...
## Restore mappings são a espinha dorsal
...
## Conteúdo dependente do usuário
...
## Links codificados são diferentes de files
...
O que discutimos em Blocos continua valendo: tabela própria de um block não entra no backup apenas porque contém `blockinstanceid`.
...
O restore precisa decidir como recriar instâncias e matrículas, respeitando opções de usuários e políticas do método. Isso foi discutido em Plugins de Matrícula e aqui fica claro por que `enrol` não pode simplesmente exportar `user_enrolments` como tabela comum.
...
## Question types e Question Engine

Questões são um caso mais complexo porque podem ser compartilhadas por diferentes atividades e organizadas em categorias. O subsistema de questões possui integração própria com backup para transportar categorias, definições das questões, respostas, usages e os dados adicionais exigidos por cada tipo.

Um qtype implementa pontos próprios para seus options, answers e estruturas adicionais, em vez de tratar as tabelas de questões como dados privados de uma activity.

Question Engine e Quiz já mostrou por que copiar `question.id` cru entre instalações seria desastroso.
...
## Erro comum com `backup_nested_element`
...
## Source e fields precisam bater
...
## Não inclua IDs derivados desnecessários
...
## Dados externos não deveriam ser copiados cegamente
...
## Teste de backup isolado da activity
...
## O que verificar depois do restore
...
## Teste com versão mais nova do plugin
...
em PHPUnit vamos aprofundar a infraestrutura PHPUnit, porém vale desde já criar o código de produção de forma que o estado restaurado seja fácil de validar.

## Não teste comparando IDs
...
Os mesmos princípios de Banco de Dados e XMLDB e de Cache e Performance continuam valendo.
...
## Restore idempotente não significa executar duas vezes
...
## Não altere `.mbz` manualmente para corrigir plugin
...
## O que eu procuro em code review

Quando reviso backup de um plugin, começo procurando tabelas próprias e classificando quais são configuração, dados filhos e dados de usuário. Depois procuro todos os campos que referenciam IDs externos à tabela e verifico annotations ou mappings. Em seguida olho file areas e links internos.
...
## Exercício - backup no site A e restore no site B
