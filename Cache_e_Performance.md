...
## O que custa caro em uma requisição Moodle
...
Moodle também possui mecanismos de invalidação por evento de cache com `cache_helper::purge_by_event()`, que são anteriores à Events API e permitem que várias definições declarem dependência de uma mesma sinalização. Não confunda isso com eventos de domínio discutidos em Events e Callbacks.
...
## Por que cachear sem estratégia de invalidação causa bugs
...
## TTL não deveria ser sua primeira solução
...
## `cachedir`, `localcachedir` e caches localizados
...
## Recordsets - retomada de Banco de Dados e XMLDB
...
## Índices - retomada de Banco de Dados e XMLDB
...
## Performance de `lib.php`

em Primeiro Plugin Corretamente vimos por que `lib.php` deve permanecer pequeno. Performance é mais uma razão.
...
A solução normalmente é recolher identificadores, registrar estado mínimo e enfileirar Adhoc Task, como vimos em Cron, Tasks e Processamento Assíncrono.
...
Em infraestrutura com Redis, métricas de hit, memória, eviction, conexões e latência ajudam a entender comportamento. Dentro do Moodle, a configuração de cache e ferramentas administrativas mostram stores e mappings, enquanto observabilidade externa pode completar a visão.
...
## Uma metodologia prática para página lenta
...
## Exercício - receber uma página lenta e identificar os gargalos
...
Request cache serve ao request. Session cache serve à sessão. Application cache compartilha estado reutilizável pela aplicação. Store é infraestrutura e deveria ser escolhida pela instalação, não hardcodeada pelo plugin. Redis pode participar da arquitetura, mas continuam sendo backend, não API de negócio.
