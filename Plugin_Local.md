...
Este capítulo junta boa parte do que vimos até aqui e coloca tudo dentro de um projeto completo, porque um plugin `local` de verdade raramente vive apenas de `version.php`, `settings.php` e uma página PHP. Ele pode ter banco, capabilities, eventos, callbacks, Tasks, cache, Web Services, integrações externas, páginas administrativas, templates Mustache e regras de negócio, e o ponto importante é fazer tudo isso sem transformar o plugin em um grande `lib.php` com milhares de linhas.

## O que realmente é um plugin local

Um plugin `local` é um componente instalado dentro de `local/nome_do_plugin`, com Frankenstyle `local_nome_do_plugin`. Ele participa do sistema de plugins do Moodle como qualquer outro componente, possui `version.php`, arquivo de idioma, namespaces, autoload, acesso ao banco, Events API, callbacks, Tasks, cache, serviços externos e demais APIs públicas que vimos nos capítulos anteriores.
...
## Quando eu criaria um plugin local
...
## Quando eu não criaria um plugin local
...
## "Mas Kraus, você só cria plugin local para quase tudo"
...
## A pergunta correta antes de criar a pasta
...
## version.php continua declarativo
...
## O lib.php deve continuar pequeno
...
```php
function local_institutionhub_extend_navigation(global_navigation $navigation) {
    \local_institutionhub\navigation\manager::extend($navigation);
}
```

Antes de colocar lógica em `lib.php`, verifique se existe Event, observer ou callback específico para aquela integração e mantenha o arquivo o menor possível.

O que eu evitaria é colocar um service inteiro dentro de `lib.php` só porque a primeira chamada chegou por callback.

## settings.php e configurações administrativas
...
Também não altere arquivos do core para adicionar menu. Se o resultado pode ser obtido por Navigation API ou callback disponível, use o contrato público. Alterar `lib/navigationlib.php` ou template do core para inserir um link institucional pode parecer rápido hoje, mas vira uma dívida a cada atualização.

## Capabilities em plugin local
...
## Capability não substitui propriedade do dado
...
Esse ponto é especialmente importante em plugin institucional porque ele tende a viver muitos anos. Um `local` usado internamente pode não passar pelo diretório de plugins, mas continua precisando de upgrades confiáveis. Às vezes isso é ainda mais importante, porque a tabela pode ter milhões de registros reais e não apenas dados de demonstração.
...
## Interface sem renderer.php desnecessário

O que discutimos em Output API e Mustache vale integralmente aqui. Não existe motivo para um plugin `local` novo ganhar `renderer.php` por tradição.
...
Renderer continua existindo no Moodle e há situações de compatibilidade e override em que ele faz sentido, mas criar `renderer.php` automaticamente em todo `local` novo adiciona uma camada que, na maioria dos casos simples, o Mustache já torna desnecessária.

## Events API dentro do plugin local
...
final class user {
    public static function updated(\core\event\user_updated $event) {
...
## Event representa algo que já aconteceu

Se você precisa saber que algo aconteceu, Event é natural. Para extensões que não representam fatos passados, use os callbacks e APIs específicas disponíveis no Moodle 3.5.
...
No Moodle 3.5, `blocking` ainda faz parte da definição de Scheduled Tasks, mas não use esse campo como única proteção de concorrência da sua regra de negócio. Quando duas execuções simultâneas seriam perigosas, desenhe a operação de forma idempotente e use Lock API ou estado persistente quando necessário.
...
## Web Services próprios

Se outro sistema precisa chamar o Moodle, um plugin `local` pode registrar External Functions em `db/services.php`. No Moodle 3.5 a implementação tradicional fica em `externallib.php`, estendendo `external_api`.

```php
require_once($CFG->libdir . '/externallib.php');

class local_institutionhub_external extends external_api {
    public static function user_status_parameters() {
        return new external_function_parameters([
            'userid' => new external_value(PARAM_INT, 'User ID'),
        ]);
    }

    public static function user_status($userid) {
        $params = self::validate_parameters(self::user_status_parameters(), [
            'userid' => $userid,
        ]);
    ...
    public static function user_status_returns() {
...
## Moodle chamando sistemas externos
...
## Dependências com outros plugins
...
Quando a comunicação entre componentes puder acontecer por Event ou callback sem acoplamento direto, isso pode ser melhor. Quando uma chamada direta é parte real do contrato, prefira dependência explícita a um `class_exists()` espalhado por cinquenta lugares.

## Arquitetura de um projeto completo
...
## Local não quer dizer global o tempo inteiro
...
## Local não significa código executado em toda página
...
Um observer roda quando o evento correspondente acontece. Uma Scheduled Task depende do cron. Uma página roda quando é acessada. Um callback em `lib.php` roda quando o core chama aquele callback. Um módulo JavaScript roda quando a página o carrega.
...
## Quando preciso executar algo ao acessar qualquer atividade
...
Callbacks de navegação e observers muito frequentes não devem fazer consultas repetidas sem necessidade. Use carregamento preguiçoso, cache quando houver estratégia de invalidação e Tasks para trabalho pesado.
...
## Segurança de plugin institucional
...
Este livro assume Moodle 3.5 e PHP 7.0 ou superior dentro da faixa suportada pela versão. Ao copiar exemplos de plugins externos, confirme se as classes, funções e arquivos usados já existem na branch `MOODLE_35_STABLE`; documentação de outra versão não é contrato para esta edição.

## Um exemplo de fluxo de instalação
...
## Passo 7, reprocessamento via AJAX
...
Onde está a chamada externa? Onde a regra de sincronização vive? Onde a página prepara dados? Onde permissionamento é declarado? Onde Tasks são registradas? Onde cache é definido? Onde schema evolui? Onde observers e callbacks são declarados?
