# Cron, Tasks e processamento assíncrono
...
No Moodle 3.5, trabalho recorrente e processamento em background devem usar a Task API, com Scheduled Tasks e Adhoc Tasks conforme o caso.

## Por que o cron deve rodar frequentemente
...
O campo `blocking` existe nas Scheduled Tasks do Moodle 3.5, mas deve ser usado com cuidado porque uma tarefa bloqueante pode atrasar todo o restante do cron.

## Diretório `classes/task/`
...
## Configuração administrativa da frequência
...
O sistema de tasks do Moodle 3.5 registra falhas e controla novas tentativas de execução. O ponto importante para o plugin é lançar a exceção quando a tarefa realmente falhou, registrar informação suficiente para diagnóstico e desenhar a operação para poder ser repetida sem duplicar efeitos.
...
## Evitando dois workers processarem o mesmo registro
...
## Limpeza de caches em tarefas longas
...
## O que aconteceu com blocking tasks
...
## Cron legado e Task API

Plugins mais antigos podem trazer callbacks de cron anteriores à Task API. No Moodle 3.5 você ainda pode encontrar esse código durante manutenção, mas para desenvolvimento novo a Task API, disponível desde o Moodle 2.7, é o caminho normal para Scheduled Tasks e Adhoc Tasks.

Ao encontrar um tutorial baseado apenas em `[modname]_cron()`, confirme o contexto histórico antes de copiar o padrão para código novo.
...
## Não transforme task em request sem navegador
...
## Uma fila não substitui transação
...
Banco de Dados e XMLDB tratou transações no banco. Aqui a lembrança importante é que background processing não suspende regras de consistência, ele apenas muda o processo que executa o código.
...
## Exercício - importar milhares de registros sem travar uma requisição HTTP
