...
## A classe principal e block_base
...
## Não monte a interface inteira dentro de get_content()
...
Quando você mistura HTML com PHP perde legibilidade, dificulta override pelo tema, aumenta risco de escaping incorreto e torna a manutenção visual muito pior. Output API e Mustache já discutiu por que a camada de apresentação deve ficar em template e o mesmo raciocínio vale aqui. O bloco não ganha uma exceção arquitetural só porque o HTML cabe em cinco linhas.
...
Não precisa criar `renderer.php` para isso. Seria apenas mais uma camada chamando um template, exatamente o tipo de boilerplate que evitamos em Output API e Mustache.
...
## Curso, atividade, Dashboard e administração não são a mesma coisa
...
## Configuração global e configuração por instância
...
## config_instance.html e por que você ainda pode encontrar isso
...
No Moodle 3.5, a configuração por instância deve ser implementada em `edit_form.php`, usando uma classe que estende `block_edit_form`. Então, se você está criando código novo e alguém sugere começar por `config_instance.html`, provavelmente está repetindo um padrão de versões antigas do Moodle.
...
## Como a configuração da instância é armazenada
...
## Capabilities de um block plugin
...
## Adicionar o bloco não significa poder usar tudo dentro dele
...
Isso volta a Segurança. A interface pode esconder um botão, mas a página ou endpoint que recebe a ação precisa verificar novamente login, contexto, capability e, se houver objeto específico, propriedade ou vínculo com aquele objeto.

## Onde o bloco aparece
...
## Cuidado com cache quando o conteúdo depende do usuário
...
A regra é a mesma de Cache e Performance. A chave do cache precisa representar todas as dimensões que alteram o resultado, como curso, usuário, grupo, idioma ou configuração, dependendo da consulta.

## Cache do conteúdo do bloco
...
## Evite trabalho pesado em get_content()
...
Quando existe interação real no frontend, use módulos AMD em `amd/src/`, que é a arquitetura recomendada para JavaScript novo no Moodle 3.5.
...
## Não coloque JavaScript inline em get_content()
...
## Output class quando o conteúdo cresce
...
O importante é não assumir caminho físico em moodledata. Use `get_file_storage()`, `stored_file` e `pluginfile()` conforme vimos em Files API e Moodledata.

## pluginfile() em block plugin
...
## Editor com arquivos na configuração da instância
...
A API de backup possui tarefas específicas para blocks e trabalha com estrutura, file areas e dados configurados da instância. Vamos aprofundar todo esse mecanismo em Backup e Restore, então aqui o objetivo é apenas não cair na falsa ideia de que backup de bloco acontece automaticamente para qualquer dado inventado pelo plugin.

## Backup de configuração não é backup das suas tabelas
...
## Remoção da instância e limpeza de dados
...
## Bloco não é lugar para cron improvisado
...
## Bloco não deve virar mini aplicação inteira
...
## Estrutura de um bloco um pouco maior
...
## O bloco como camada de integração visual
...
## Exercício - criar um bloco configurável e contextual
