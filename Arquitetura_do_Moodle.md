...
## `config.php` e inicialização do ambiente
...
É comum acessar propriedades como `$CFG->wwwroot`, `$CFG->dirroot`, `$CFG->dataroot`, `$CFG->libdir` e várias opções globais. Em código antigo também há muitos acessos diretos a propriedades configuradas no banco, porque esse era o padrão durante anos, mas em código de plugin você deve preferir `get_config()` quando estiver lidando com configuração do próprio componente. Isso deixa explícito de onde o dado vem e evita transformar `$CFG` em uma sacola onde todo mundo coloca alguma coisa.
...
## O que acontece em `lib/setup.php`
...
Seu código deve entrar pelo `config.php` ou pelas APIs previstas para aquele tipo de execução e deixar o Moodle cuidar da própria inicialização. Não tente reproduzir partes de `lib/setup.php` dentro do plugin para criar um bootstrap particular.
...
## As globais `$DB`, `$PAGE`, `$OUTPUT`, `$USER`, `$COURSE`

O Moodle 3.5 usa classes e namespaces em muitas APIs, mas ainda convive com globais muito importantes, e tentar fingir que elas não existem só porque "global é feio" não ajuda. O caminho melhor é entender o papel de cada uma e não usá-las como se fossem intercambiáveis.
...
## O ciclo de vida de uma página Moodle
...
### require_login() sem curso não transforma a página em página de curso
...
### O que muda quando usamos require_login($course)
...
### Por que a navegação e a aparência mudam
...
### require_login($course, false) e o segundo parâmetro que muita gente interpreta errado
...
### Não remova o curso de require_login() apenas para "consertar" o layout
...
O erro que eu evitaria é memorizar caminho como regra eterna. Seu plugin deve usar APIs de descoberta e variáveis como `$CFG->dirroot` quando precisa localizar algo conhecido, em vez de assumir uma topologia completa montada com `dirname()`.
...
## Web root e por que arquivos internos não devem ficar diretamente expostos

Web root é a pasta a partir da qual o servidor web resolve os caminhos públicos. Em uma instalação tradicional do Moodle 3.5, a própria raiz do Moodle fica sob o servidor web, enquanto o diretório definido em `$CFG->dataroot` deve ficar fora dessa árvore e nunca deve ser servido diretamente.
...
## `core`, `mod`, `local`, `block`, `tool` e demais componentes
...
`mod` representa atividades, `block` representa blocos, `tool` representa ferramentas administrativas e `local` existe para funcionalidades que realmente não se encaixam melhor em um tipo específico. Além deles há `auth`, `enrol`, `theme`, `report`, `format`, `filter`, `repository`, `availability`, `qtype`, `qbehaviour`, `qformat`, `gradereport` e vários outros pontos de extensão.
...
No Moodle 3.5, classes colocadas nos locais esperados podem ser carregadas automaticamente. Quando o PHP encontra `new \local_relatorios\service\report_builder()` e a classe ainda não existe na memória, o autoloader registrado durante o bootstrap analisa o nome, identifica o componente `local_relatorios`, converte o restante do namespace para o caminho sob `classes/` e inclui o arquivo correspondente.
...
Dentro de `classes/` você pode criar subdiretórios que representem responsabilidades reais. `classes/service/`, `classes/output/`, `classes/task/`, `classes/event/` e `classes/local/` são exemplos possíveis, sendo que diretórios como `task/`, `event/` e `output/` costumam acompanhar responsabilidades reconhecíveis pelas APIs do Moodle. O caminho precisa acompanhar o namespace e o nome do arquivo acompanha o nome da classe conforme as regras de coding style.
...
## Relação entre contexto, capability e acesso
...
```php
$CFG->dirroot aponta para a raiz do código Moodle relevante à aplicação e é usado quando você realmente precisa montar um caminho absoluto para algum arquivo conhecido. No Moodle 3.5, `$CFG->dirroot` aponta para a raiz do código da instalação.
```
...
## `filedir`, `temp`, `cache`, `localcache`, `sessions` e `trashdir`
...
## Por que Moodledata não pode ser servido diretamente pelo servidor web
...
## Como localizar uma API no código do Moodle
...
## Como usar a documentação PHPDoc do core
...
Não use apenas a busca por nome e pare na primeira classe parecida. Confira o componente, a documentação correspondente ao Moodle 3.5 e a herança. Existem nomes semelhantes em plugins diferentes e a branch de desenvolvimento do repositório pode conter código que ainda não faz parte da versão instalada.
