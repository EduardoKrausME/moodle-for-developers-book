...
## 19.2 Autenticação, autorização e matrícula
...
## 19.3 Estrutura de um plugin `auth`

Os plugins de autenticação carregam bastante história do Moodle. Você ainda encontra implementações clássicas cuja classe principal fica em `auth.php`, enquanto partes mais recentes do core usam classes namespaced em `classes/`. A branch mínima suportada pelo projeto importa bastante aqui.
...
## 19.5 O construtor não é lugar para autenticar
...
## 19.7 Um `auth` externo baseado em senha
...
## 19.8 Não faça a integração institucional inteira em `user_login()`
...
## 19.9 Usuário externo continua tendo registro local
...
## 19.12 Nunca copie a senha externa para o Moodle
...
## 19.16 SSO pode significar coisas completamente diferentes
...
## 19.17 SSO com usuário e senha ainda é autenticação delegada
...
## 19.18 SSO federado com OIDC ou SAML
...
## 19.19 "Mas Kraus, eu já tenho uma área do aluno. Quero só um link que entre no Moodle logado"
...
## 19.21 O fluxo correto de um link de acesso único
...
## 19.22 Não coloque `userid` ou email em uma URL e chame isso de SSO
...
## 19.23 A identidade do CMS não é a identidade do aluno
...
## 19.25 Por que uso único importa
...
## 19.28 Onde esse mecanismo deve morar
...
## 19.29 Quando eu colocaria isso fora de um `auth`
...
## 19.30 Quando um `auth` específico ainda pode fazer sentido
...
## 19.31 CMS confiável não significa navegador confiável
...
## 19.32 Web Service para emitir a URL
...
## 19.33 Capability para emissão de login
...
## 19.35 Não troque automaticamente o método de autenticação do usuário
...
## 19.36 Logout entre CMS e Moodle
...
## 19.37 Criação automática de conta
...
## 19.38 Atualização automática de conta
...
## 19.40 `get_userinfo()` e criação local
...
## 19.43 Scheduled Tasks para sincronização
...
A task deve trabalhar em lotes, ser idempotente e registrar progresso, retomando os princípios do Capítulo 11.

## 19.44 Usuário removido da fonte
...
## 19.52 Hooks históricos de login

`auth_plugin_base` expõe Hooks históricos como `pre_loginpage_hook()`, `loginpage_hook()`, `pre_user_login_hook()` e `user_authenticated_hook()`.
...
## 19.53 Provedores de identidade na tela de login
...
## 19.58 MFA não é um plugin `auth`

MFA é outra camada. Ele não responde qual backend autentica a conta, mas exige provas adicionais depois ou durante o processo de autenticação principal.

No Moodle moderno, MFA possui infraestrutura própria com `tool_mfa` e plugins de fator. Portanto, se a necessidade é TOTP, email, security key ou regra contextual, criar `auth_meumfa` é conceitualmente errado.

O usuário pode autenticar pelo `auth_academicsso` e depois ser submetido ao MFA do Moodle.

## 19.59 MFA não corrige primeiro fator inseguro

Adicionar um segundo fator não compensa token reutilizável, segredo exposto, open redirect, ausência de TLS ou vínculo incorreto de identidade.

Cada camada precisa ser segura isoladamente.

## 19.60 Identificador externo estável
...
## 19.63 Login token do formulário não é token de SSO

O login form do Moodle possui mecanismos próprios de proteção e um `logintoken` usado no fluxo da página. Isso não é uma credencial genérica para um CMS autenticar usuários externamente.
...
## 19.66 Mensagem pública e log técnico
...
## 19.71 Cliente externo separado
...
## 19.72 Reaproveitando os dados do login
...
## 19.73 Fluxo completo com CMS e Moodle
...
## 19.74 Fluxo completo com `auth` por senha externa
...
## 19.75 O que não deve estar num plugin `auth`
...
## 19.77 Exercício - dois SSOs diferentes
