...
## 28.1 Segurança ofensiva aplicada ao desenvolvimento
...
## 28.2 O laboratório precisa ser descartável
...
## 28.3 O objetivo não é "achar CVE"
...
## 28.6 Faça um inventário antes de testar
...
## 28.8 Tipo válido não significa objeto autorizado
...
## 28.10 Identifique decisões de autorização
...
## 28.11 Identifique arquivos expostos
...
## 28.13 Revisão orientada a fluxo
...
## 28.14 Laboratório de SQL Injection
...
## 28.15 Como comprovar SQL Injection com segurança
...
## 28.17 Não tente corrigir SQL Injection com `addslashes()`
...
## 28.21 `format_string()` não é `s()` com outro nome
...
## 28.24 Triple Mustache é um ponto de revisão
...
## 28.27 `sesskey` não substitui capability
...
## 28.33 Patch de context confusion
...
## 28.34 Manipulação de `contextid`
...
## 28.35 Derive contexto, não aceite como verdade
...
## 28.40 Patch de LFI com allowlist
...
## 28.41 `basename()` não resolve todo problema de include
...
## 28.45 Patch de file disclosure
...
## 28.49 Não confie apenas na extensão
...
## 28.52 Patch de AJAX tradicional
...
## 28.53 External Function insegura

External Function vulnerável:
...
Mesmo que os parâmetros estejam tipados em PHP, faltam os contratos da External API.

## 28.54 Patch de External Function

O fluxo correto começa validando parâmetros, depois contexto e capability:
...
Em External Functions não substitua `validate_context()` por `require_login()`.

## 28.55 `db/services.php` não protege a função
...
## 28.56 Sesskey ausente em ação administrativa
...
## 28.57 Capability no contexto errado
...
## 28.59 O risco de SSRF no Moodle
...
## 28.60 Patch de SSRF com destinos controlados
...
## 28.61 Redirect pode reabrir SSRF
...
## 28.66 Não logue o request inteiro
...
## 28.68 Corrigindo race condition
...
## 28.69 Lock sem timeout vira outro problema
...
## 28.72 Download sem autorização
...
## 28.73 Autorização na fronteira
...
Privacy leak não precisa ser uma vulnerabilidade clássica de acesso. Pode ser um endpoint autorizado que devolve dados demais.

Exemplo: uma função retorna lista de alunos com `email`, `idnumber`, IP e último acesso quando a tela só precisa de nome e status.

Menos dado retornado significa menos superfície de vazamento.

## 28.75 `SELECT *` também merece revisão de privacidade

`SELECT *` não é automaticamente vulnerável, mas em endpoints externos e exports pode facilitar exposição acidental quando a tabela ganha novos campos sensíveis.

Selecione explicitamente aquilo que a resposta realmente precisa.

## 28.76 Logs e privacy leak

Um plugin pode proteger perfeitamente a interface e ainda gravar payload completo de integrações contendo CPF, email, token ou conteúdo acadêmico em logs permanentes.

Logs também são dados. Defina retenção, acesso, redaction e finalidade.

## 28.77 `unserialize()` com entrada não confiável
...
## 28.82 Email injection e templates
...
## 28.84 `PARAM_FILE` não torna shell seguro
...
## 28.85 Informação sensível em exception
...
## 28.86 Debug mode não é licença para vazar secret
...
## 28.88 Grupos como fronteira de dados
...
## 28.89 Curso visível não significa atividade acessível
...
## 28.90 Teste com usuário mínimo
...
## 28.91 Teste horizontal e vertical
...
## 28.94 Teste parâmetros extras e ausentes
...
## 28.96 Automatize regressões de segurança
...
## 28.97 PHPUnit para autorização
...
## 28.98 Behat para interface de segurança
...
## 28.99 Static analysis e grep ajudam, mas não substituem revisão
...
## 28.100 Plugin Validate não é scanner de segurança completo
...
## 28.101 Revisão de endpoints externos
...
## 28.104 Revisão de integrações HTTP
...
## 28.106 Revisão de Privacy

Não limite segurança a invasão. Verifique se exports devolvem dados demais, se delete apaga o usuário correto, se logs e arquivos estão cobertos e se dados enviados externamente foram declarados.

Uma falha de privacidade pode existir mesmo quando todas as capabilities estão certas.

## 28.107 Como escrever o relatório
...
## 28.110 Não corrija vulnerabilidade com obscuridade
...
## 28.112 Projeto do laboratório `local_vulnlab`
...
## 28.113 Não misture todas as falhas no mesmo endpoint
...
## 28.114 Branch vulnerável e branch corrigida
...
## 28.115 Exercício final - auditoria
...
## 28.116 Exercício final - falhas mínimas esperadas
...
privacy -> reduzir dado e cobrir provider
```

## 28.118 Exercício final - testes de regressão
...
## 28.119 Exercício final - revisão cruzada
...
## 28.120 O que este capítulo deve mudar na forma de programar
