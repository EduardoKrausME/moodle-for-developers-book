...
![Segurança no Moodle](image/cap08-seguranca-moodle.png)
...
## 8.1 Modelo de segurança do Moodle
...
### 8.3.1 require_login() e require_login($course) não são equivalentes
...
### 8.3.2 Passar o curso é uma decisão de acesso, não uma técnica de breadcrumb
...
### 8.3.3 O que o segundo parâmetro realmente controla
...
### 8.3.4 Quando passamos o course module
...
### 8.3.5 require_login() continua não sendo require_capability()
...
### 8.3.6 Três padrões que evitam confusão
...
## 8.4 Contextos - retomada do Capítulo 1
...
![Permissões do professor no Moodle](image/cap08-permissoes-do-professor.png)
...
## 8.10 Capability no contexto correto
...
## 8.11 Capability versus propriedade do registro
...
SQL Injection acontece quando entrada controlável altera a estrutura da consulta em vez de permanecer como valor. No Moodle moderno, a maior parte das operações comuns pode ser feita com DML API sem escrever SQL, e quando SQL manual é necessário existem placeholders justamente para separar comando e dados.
...
## 8.24 Por que a DML API não resolve SQL Injection se for utilizada incorretamente
...
## 8.32 Quando `PARAM_RAW` é perigoso
...
## 8.38 MIME type versus extensão
...
## 8.40 Web Service authorization - aprofundado no Capítulo 14
...
O fluxo típico é `validate_parameters()`, resolução do contexto real, `validate_context()` e depois `require_capability()` ou outra regra de autorização. O Capítulo 14 entra em detalhes sobre estruturas de parâmetros, retornos, serviços e tokens, mas a regra de segurança precisa aparecer desde já: uma função estar registrada em `db/services.php` não significa que ela pode confiar no chamador.
...
No Moodle moderno, o caminho preferido para AJAX é `core/ajax` chamando External Functions marcadas para AJAX. Isso é bom porque você herda o contrato de parâmetros e retorno da External API, mas não significa que a chamada ficou automaticamente autorizada.
...
## 8.42 `contextid` vindo do cliente
...
## 8.45 Moodle Curl API para chamadas externas
...
Defina timeouts. Uma chamada sem limite pode prender workers PHP e transformar indisponibilidade de um serviço externo em indisponibilidade do Moodle. Em integrações importantes, combine isso com Task API, retry controlado e idempotência, assuntos aprofundados no Capítulo 11 e no Capítulo 14.

## 8.46 Secrets e tokens
...
## 8.47 Por que não guardar senha/API key em código-fonte
...
## 8.48 Logs sem exposição de informações sensíveis
...
## 8.49 Forum - Capability é suficiente para proteger uma página?
...
## 8.50 Uma ordem prática para revisar segurança
