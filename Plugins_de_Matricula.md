...
## O que é um plugin de matrícula
...
## Quando escolher `enrol` em vez de `local`
...
Dependendo do projeto, entram `classes/task/`, `db/tasks.php`, tabelas próprias, serviços, eventos, formulários, arquivos de backup. Diferentemente de vários plugins em que a classe principal fica em `classes/`, o contrato histórico do tipo `enrol` ainda exige a classe base do plugin em `lib.php`.

## A classe `enrol_plugin`
...
## Plugin e instância não são a mesma coisa
...
## Matrícula e atribuição de papel são conceitos diferentes
...
## É possível ter papel sem matrícula
...
## Como o Moodle decide se uma matrícula está ativa
...
É diferente de suspender um usuário específico. Se uma instância que representa a turma `ERP-2018-03` for desabilitada, o problema é a origem inteira naquele curso; se apenas um contrato foi bloqueado, o status deveria ser alterado no `user_enrolments` daquele usuário.
...
## `ENROL_USER_ACTIVE` e `ENROL_USER_SUSPENDED`
...
```php
$timestart = strtotime('2018-10-01 00:00:00');
$timeend = strtotime('2018-12-31 23:59:59');
```
...
## Obtendo o plugin com `enrol_get_plugin()`
...
## Obtendo instâncias com `enrol_get_instances()`
...
## O papel informado em `enrol_user()`
...
## Atualizando a matrícula com `update_user_enrol()`
...
## Remover matrícula não é apagar o usuário
...
## Plugins totalmente automáticos
...
## Estratégias quando o usuário desaparece da origem
...
## Sincronização por Scheduled Task
...
Se o processo for grande, utilize as estratégias de Cron, Tasks e Processamento Assíncrono, como lotes, retomada, logs, lock e eventualmente adhoc tasks para dividir trabalho.

## A task precisa ser idempotente
...
```
GET /enrolments?updated_after=2018-05-17T18:00:00
```
...
## Coorte não é turma de curso automaticamente
...
## Nunca grave diretamente em `{user_enrolments}` para "ganhar tempo"
...
## Não confunda gateway de pagamento com método de matrícula
...
## Pagamento confirmado deve ser idempotente
...
O princípio é o mesmo das tasks de Cron, Tasks e Processamento Assíncrono e dos webhooks de Web Services e Integrações.
...
Mas lembre da regra de Events e Callbacks: Event representa um fato ocorrido. Não use observer para tentar impedir uma matrícula que já deveria ter sido barrada pelo próprio método ou pela autorização anterior.

## O observer não deve virar uma segunda fonte de matrícula
...
## Backups precisam entender a origem da matrícula
...
## Restaurando matrículas de usuários
...
## `find_instance()` e criação de cursos por CSV
...
## Segurança em plugins de matrícula
...
## Um fluxo completo para `enrol_contractsync`

Vamos imaginar a sequência de uma sincronização madura. A task busca alterações no ERP desde o último cursor e encontra que o usuário externo `A9842` passou a ter contrato ativo na turma `T2018-03`. O plugin resolve `A9842` para um `userid`, resolve a turma para uma instância `enrol_contractsync`, procura a matrícula existente e não encontra.
...
## O que pertence ao plugin e o que pertence ao core
...
## Exercício - matrícula sincronizada com sistema externo
